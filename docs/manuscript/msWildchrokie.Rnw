...
\Sexpr{fmt_stat(b_gdd_per, "B. papyrifera")} \%/GDD), Grey birch (\textit{Betula populifolia}; $\beta =$
\Sexpr{fmt_stat(b_gdd_per, "B. populifolia")} \%/GDD), and Yellow birch (\textit{Betula alleghaniensis}; $\beta =$
\Sexpr{fmt_stat(b_gdd_per, "B. alleghaniensis")} \%/GDD), grey alder (\textit{Alnus incana}) trended in the same direction ($\beta =$ 
\Sexpr{fmt_stat(b_gdd_per, "A. incana")} \%/GDD); Figure \ref{fig:wcmuplotbspp}a and e; Table\ref{tab:wc_slope_table}).
Unlike its response to warmer thermal season, {\Ainc} grew more with longer calendar seasons ($\beta =$
\Sexpr{fmt_stat(b_gsl_per, "A. incana")} \%/day). {\Bpap} also grew more with longer calendar seasons ($\beta =$ 
\Sexpr{fmt_stat(b_gsl_per, "B. papyrifera")} \%/day), while the other two \textit{Betula} did not, though they trended in the same way ({\Ball}: $\beta =$
\Sexpr{fmt_stat(b_gsl_per, "B. alleghaniensis")} \%/day, and {\Bpop}: $\beta =$
\Sexpr{fmt_stat(b_gsl_per, "B. populifolia")} \%/day; Figure \ref{fig:wcmuplotbspp}b and f; Table \ref{tab:wc_slope_table}). 
...
\Sexpr{fmt_stat(b_sos_per, "A. incana")} \%/day; Figure \ref{fig:wcmuplotbspp}c and g; Table \ref{tab:wc_slope_table}). {\Bpop} trended in the opposite direction ($\beta =$
\Sexpr{fmt_stat(b_sos_per, "B. populifolia")} \%/day; Figure \ref{fig:wcmuplotbspp}c and g; Table \ref{tab:wc_slope_table}). {\Ball}, which did not grow more with longer calendar season, grew less with an earlier start of season ($\beta =$
\Sexpr{fmt_stat(b_sos_per, "B. alleghaniensis")} \%/day; Figure \ref{fig:wcmuplotbspp}c and g; Table \ref{tab:wc_slope_table})---but more with a later end of season ($\beta =$ 
\Sexpr{fmt_stat(b_eos_per, "B. alleghaniensis")} \%/day). Alongside {\Ball}, two other species grew more with a later end of season ({\Bpop}: $\beta =$
\Sexpr{fmt_stat(b_eos_per, "B. populifolia")} \%/day, and {\Bpap}: $\beta =$
\Sexpr{fmt_stat(b_eos_per, "B. papyrifera")} \%/day; Figure \ref{fig:wcmuplotbspp}d and h; Table \ref{tab:wc_slope_table}), with the latter being the only one of those three that also grew more with longer calendar season. The estimates were unchanged when using the full (SOS: $n =$
...
\Sexpr{fmt_stat(b_eosgain_mu, "all")}\%,  compared to an increase of XX% from an earlier start of spring (estimated using our model of GDD shown in cite fig, tables, see methods for more details)
Across species, extending the season by seven days in the autumn increased growth by 
\Sexpr{fmt_stat(b_eosgain_mu, "all")} \% compared to 
\Sexpr{fmt_stat(b_sosgain_mu, "all")} \% by extending it in the spring ({\Ainc}, 
