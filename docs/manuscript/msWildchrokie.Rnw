...
%-------------------------------------------------------------------------------
We did not find any clear effect of year on growth (Table \ref{tab:tab_GDD}), and there were no apparent differences in baseline growth between species (Figure \ref{fig:wcmuplotasite}b; Table \ref{tab:tab_GDD}). The limited variation between provenances ($\sigma = $
...
\Sexpr{sprintf("%.2f", exp(sigma_df2$p95[sigma_df2$sigma == "sigma_atreeid"]))}); Table \ref{tab:tab_GDD}; Figure \ref{fig:wcmuplotatreeid}).
