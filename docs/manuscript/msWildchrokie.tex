...
, UI:
, 
 RW/GDD), Grey birch (\textit{Betula populifolia}; $\beta =$
, UI: 
, 
 RW/GDD), and Yellow birch (\textit{Betula alleghaniensis}; $\beta =$
, UI: 
, 
 RW/GDD), grey alder (\textit{Alnus incana}) trended in the same direction ($\beta =$ 
, UI: 
, 
 RW/GDD); Figure \ref{fig:wcmuplotbspp}a and e; Table\ref{tab:wc_slope_table}).
Unlike its response to warmer thermal season, {\Ainc} grew more with longer calendar seasons ($\beta =$
, UI: 
,
 RW/day). {\Bpap} also grew more with longer calendar seasons
($\beta =$ , UI: 
, 
 RW/day), while the other two \textit{Betula} did not, though they trended in the same way ({\Ball}: $\beta =$
, UI: 
, 
 RW/day, and {\Bpop}: $\beta =$
, UI: 
, 
 RW/day; Figure \ref{fig:wcmuplotbspp}b and f; Table \ref{tab:wc_slope_table}). 
...
, UI:
, 
 RW/day; Figure \ref{fig:wcmuplotbspp}c and g; Table \ref{tab:wc_slope_table}). 
{\Bpop} trended in the opposite direction ($\beta =$
, UI:
, 
 RW/day; Figure \ref{fig:wcmuplotbspp}c and g; Table \ref{tab:wc_slope_table}). {\Ball}, which did not grow more with longer calendar season, grew less with an earlier start of season ($\beta =$
, UI:
, 
 RW/day; Figure \ref{fig:wcmuplotbspp}c and g; Table \ref{tab:wc_slope_table})---but more with a later end of season ($\beta =$ 
, UI:
, 
 RW/day). Alongside {\Ball}, two other species grew more with a later end of season ({\Bpop}: $\beta =$
, UI:
, 
 RW/day, and {\Bpap}: $\beta =$
, UI:
, 
 RW/day; Figure \ref{fig:wcmuplotbspp}d and h; Table \ref{tab:wc_slope_table}), with the latter being the only one of those three that also grew more with longer calendar season. The estimates were unchanged when using the full (SOS: $n =$
239 and EOS: $n =$
