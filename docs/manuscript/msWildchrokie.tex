...
0.18 (UI:
-0.80, 
1.15) ring width (log(mm); henceforth `RW'); 2019: $\alpha =$
0.17 (UI:
-0.81, 
1.13) RW; 2020: $\alpha =$
-0.44 (UI:
-1.42, 
0.53) RW; Table \ref{tab:tab_GDD}), and there were no apparent differences in baseline growth between species ({\Ainc}: $\alpha =$ 
0.95\% (UI: -3.11, 5.07) RW; {\Ball}: $\alpha =$
0.13\% (UI: -3.97, 4.32) RW; {\Bpap}: $\alpha =$
-4.04\% (UI: -8.98, 0.80) RW; {\Bpop}: $\alpha =$
-0.76\% (UI: -4.97, 3.50) RW; Figure \ref{fig:wcxyGDD} and \ref{fig:wcxyGSL}; Table \ref{tab:tab_GDD}). In addition, growth did not vary with provenance (Second College Grant (NH): $\alpha =$
-0.03 (UI:
-0.22, 
...
0.04 (UI:
-0.14, 
0.25) RW; White Mountains (NH): $\alpha =$
0.08 (UI:
-0.09, 
0.29) RW; Figure \ref{fig:wcmuplotasite}b; Table \ref{tab:tab_GDD}). The limited variation between provenances ($\sigma = $
0.17 (UI:
0.02, 
