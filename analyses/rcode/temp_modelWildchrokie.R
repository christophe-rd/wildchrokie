# Wildchrokie model
# CRD 23 April 2025
# Started in Boston shortly after completing field work for the tree sppotters

# Goal: build a model to understand the relationship between growth and growing degree days using the tree cookies collected in the wildhell common garden in sppring of 2023
...
for (i in 1:ncol(aspp_df)) { # i = 1
  ...
  aspp_df2$fit_a_spp_per5[i] <- round(quantile(aspp_df[[i]], probs = 0.055), 3)
  aspp_df2$fit_a_spp_per95[i] <- round(quantile(aspp_df[[i]], probs = 0.945), 3)
}
aspp_df2
