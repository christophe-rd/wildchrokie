...
util$check_all_hmc_diagnostics(diagnostics)
saveRDS(fit, "output/stanOutput/modelGrowthGDD_asitePerSpp")
...
asite_df2$latbi <- emp$latbi[match(asite_df2$spp, emp$spp_num)]
asite_df2$site_name <- emp$site[match(asite_df2$site, emp$site_num)]

asite_df2$mean2 <- site_df2$mean[match(asite_df2$site, site_df2$site)]
y_jit <- jitter(asite_df2$mean, amount = 0.01)  # adjust amount as needed
plot(asite_df2$mu, y_jit,
     xlab = "1 site estimte per spp",
     ylab = "general site estimate",
     main = "asite",
     type = "n",
     frame = FALSE,
     xlim = range(c(asite_df2$p25, asite_df2$p75)),
     ylim = range(c(asite_df2$p25, asite_df2$p75)))

arrows(x0 = asite_df2$p25, y0 = y_jit,
       x1 = asite_df2$p75, y1 = y_jit,
       angle = 90, code = 3, length = 0,
       lwd = 1.5, col = "darkgray")

points(asite_df2$mu, y_jit,
       pch = 16, col = wccolslatbi[asite_df2$latbi], cex = 1.5)
...
# order by species then site
asite_df2$latbi <- factor(asite_df2$latbi, levels = unique(asite_df2$latbi))
asite_df2 <- asite_df2[order(asite_df2$latbi, asite_df2$site), ]

# species colors
cols <- wccolslatbi[asite_df2$latbi]

# y positions with gaps between species
gap <- 2
asite_df2$y <- seq_len(nrow(asite_df2))

current_y <- nrow(asite_df2) + (length(unique(asite_df2$latbi))-1)*gap

for(sp in levels(asite_df2$latbi)){
  idx <- which(asite_df2$latbi == sp)
  n <- length(idx)
  
  asite_df2$y[idx] <- current_y:(current_y-n+1)
  current_y <- current_y - n - gap
}


plot(NA,
     xlim = range(c(asite_df2$p05, asite_df2$p95)),
     ylim = c(0, max(asite_df2$y)+1),
     xlab = "intercepts estimate",
     ylab = "",
     yaxt = "n",
     bty = "l")
...
# 5-95 intervals
segments(asite_df2$p05, asite_df2$y,
         asite_df2$p95, asite_df2$y,
         col = adjustcolor(cols, alpha.f = 0.6),
         lwd = 1)


# 25-75 intervals
segments(asite_df2$p25, asite_df2$y,
         asite_df2$p75, asite_df2$y,
         col = cols,
         lwd = 3)


# means
points(asite_df2$mu, asite_df2$y,
       pch = 16,
       col = cols,
       cex = 1.2)
